...
export const getUserProfile = TryCatch(async (req, res, next) => {
  ...
  const users = await sql`
    SELECT u.user_id, u.name, u.email, u.phone_number, u.role, u.bio, u.resume, u.
    resume_public_id, u.profile_pic, u.profile_pic_public_id, u.subscription,
...
export const deleteSkillFromUser = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (!skillName || skillName.trim() === "") {
      throw new ErrorHandler(400, "Please Provide skillname.");
    }
