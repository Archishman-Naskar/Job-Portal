...
import { applicationStatusUpdateTemplate } from "../tamplate.js";
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    if (user.role !== "recruiter") {
      throw new ErrorHandler(
        403,
        "Forbideen : Only recruiter can create a company.",
      );
    }
    ...
    if (existingCompanies.length > 0) {
      throw new ErrorHandler(
        409,
        `A company with the name ${name} already exist.`,
      );
    }
...
export const createJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const requiredFields = [title, description, salary, location, role, openings];
...
export const updateJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const [existingJob] =
    await sql`SELECT posted_by_recuriter_id FROM jobs WHERE job_id = ${
      req.params.jobId
    }`;
...
export const getAllApplicationForJob = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    if (user.role !== "recruiter") {
      throw new ErrorHandler(
        403,
        "Forbidden: Only recruiter can see all application for a job",
      );
    }
    ...
    const applications =
      await sql`SELECT * FROM applications WHERE job_id = ${jobId} ORDER BY
      subscribed DESC, applied_at ASC`;
...
export const updateApplication = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    const [job] =
      await sql`SELECT posted_by_recuriter_id, title FROM jobs WHERE job_id = $
      {application.job_id}`;
    ...
    const [updatedApplication] =
      await sql`UPDATE applications SET status = ${req.body.status} WHERE
